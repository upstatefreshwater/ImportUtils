...
```{r, echo=FALSE,include=TRUE,fig.align='left',fig.width=10,fig.height=5, fig.cap='Figure 1: Turbidity data as a timeseries.'}
data_stationary <- is_stationary(
  data_rename,
  stationary_secs = 22,
  start_trim_secs = 4
...
```{r, echo=FALSE,include=TRUE,fig.align='left',fig.width=10,fig.height=5, fig.cap='Figure 2: Turbidity data within the 9 m stationary block.'}
pdat <- turb_stable[[10]]
...
```{r, warning = FALSE, message = FALSE,echo=FALSE,include=TRUE,fig.align='left',fig.width=10,fig.height=5, fig.cap='Figure 2: Turbidity data within the 9 m stationary block.'}
...
```{r echo=FALSE, fig.align='left', fig.cap='Figure 3: Lagged differences in turbidity data', fig.height=5, fig.width=10, message=FALSE, warning=FALSE, include=TRUE}
dplot_dat <- 
pdat %>% 
...
So where does that leave us? I think that the path of least friction is to remove range threshold criteria for optical sensors and rely solely on 
relatively lax slope thresholding for stability detection. I think for now, we should keep slope as a tunable parameter rather than trying 
to automate something with coefficient of variation within the function which would automatically adjust thresholding. My only concern is 
how do the sensors behave as they age? Supposedly, In-Situ accounts for this, so I guess time will tell. 
