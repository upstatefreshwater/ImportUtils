...
is_stationary <- function(df,
  ...
  if(length(valid_idx) > 0){
    ...
    end_idx <- valid_idx
    # Update is_stationary_flag to TRUE using the valid_idx
    is_stationary_flag[unlist(mapply(seq, start_idx, end_idx))] <- TRUE         # Mark Sonde as stationary when the range across the window < threshold
  }
  ...
  if(drop_cols){
    df_out <- df_out |> dplyr::select(-c(is_stationary_initial, stationary_block_id, stationary_duration))
  }
